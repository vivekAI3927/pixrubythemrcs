module SurveysHelper
  ...
  def survey_scope(resource)
    if action_name =~ ACTION_NEW_CREATE
      surveys_path(resource)
    elsif action_name =~ ACTION_EDIT_UPDATE
  ...
  def is_quiz? something
    something == 0 || something == SURVEY_TYPES[0]
  end

  def is_score? something
    something == 1 || something == SURVEY_TYPES[1]
  end

  def is_poll? something
    something == 2 || something == SURVEY_TYPES[2]
  end
